...
class LowRankLinear(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, rank_ratio: float = 0.25):
        super().__init__()
        self.rank = max(1, int(min(in_features, out_features) * rank_ratio))  # 秩=25%原维度
        self.fc1 = nn.Linear(in_features, self.rank, bias=False)  # 降维
        self.fc2 = nn.Linear(self.rank, out_features, bias=True)   # 升维
