...
# 在 callback.py 中添加
class DomainAdaptationValidator(pl.callbacks.Callback):
    """分离源域/目标域验证结果并计算指标"""
    def __init__(self, source_domain_indices, target_domain_indices):  # 初始化参数名
        super().__init__()
        # 将属性名与参数名保持一致（关键修复）
        self.source_domain_indices = source_domain_indices  # 原代码此处为 self.source_indices
        self.target_domain_indices = target_domain_indices  # 原代码此处为 self.target_indices
    ...
    def on_validation_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx=0):
        x, scene_labels, domain_labels, city_labels = batch
        # 特征提取（与模型验证逻辑一致）
        x = pl_module.spec_extractor(x).unsqueeze(1) if pl_module.spec_extractor is not None else x.unsqueeze(1)
        # 验证模式下模型直接返回融合后的场景预测（单输出）
        y_hat = pl_module.backbone(x=None, x_t=x, type="valid", alpha=0.0)  # 调用backbone的验证模式
        preds = torch.argmax(y_hat, dim=1).cpu().numpy()
        domain_labels = domain_labels.cpu().numpy()
        scene_labels = scene_labels.cpu().numpy()

        # 分离源域和目标域数据
        src_mask = np.isin(domain_labels, self.source_domain_indices)
        tgt_mask = np.isin(domain_labels, self.target_domain_indices)

        # 累计预测和标签
        self.source_preds.extend(preds[src_mask])
        self.source_labels.extend(scene_labels[src_mask])
        self.target_preds.extend(preds[tgt_mask])
        self.target_labels.extend(scene_labels[tgt_mask])

    def on_validation_epoch_end(self, trainer, pl_module):
        # 计算源域/目标域准确率
        source_acc = accuracy_score(self.source_labels, self.source_preds) if self.source_labels else 0.0
        target_acc = accuracy_score(self.target_labels, self.target_preds) if self.target_labels else 0.0
        domain_gap = source_acc - target_acc  # 域差距

        # 记录到 TensorBoard
        pl_module.log("source_acc", source_acc, on_epoch=True, logger=True)
        pl_module.log("target_acc", target_acc, on_epoch=True, logger=True)
        pl_module.log("domain_gap", domain_gap, on_epoch=True, logger=True)
        print(f"源域准确率: {source_acc:.4f}, 目标域准确率: {target_acc:.4f}, 域差距: {domain_gap:.4f}")